...
'''
class CFG:
    EXPERIMENT_NAME = 'train_modernbert_base_question_regression'
...
def preprocess_dataset(sample):
    return {
        'text': sample['problem'],
        'label': float(sample['generation_token_legnth'] / 12800.)
    }

dataset_df = pd.read_pickle('input/openr1_math_220k_extended.pkl')
dataset_df = dataset_df.groupby('question_id').agg({
    'problem': 'first',
    'is_completed': 'min', 
    'is_correct': 'min', 
    'generation_token_legnth': 'mean'}).reset_index()
